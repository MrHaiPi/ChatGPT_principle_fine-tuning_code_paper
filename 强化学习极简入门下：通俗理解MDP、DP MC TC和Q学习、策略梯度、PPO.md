...
，从而可得**  
$`A\pi (s,a)=Q\pi (s,a)−V\pi (s)=R+\gamma V\pi (st+1)−V\pi (s)`$  （为什么？参考TRPO的提出：[https://zhuanlan.zhihu.com/p/26308073](https://zhuanlan.zhihu.com/p/26308073)和[https://hrl.boyuai.com/chapter/2/trpo算法/](https://hrl.boyuai.com/chapter/2/trpo算法/)）
...
A_t^{GAE} & = \sum_{l=0}^{\infty}(\gamma \lambda)^l \delta_{t+l}^V \\
          & = \delta_{t}^V + \sum_{l=1}^{\infty}(\gamma \lambda)^l \delta_{t+l}^V \\
          & = \delta_{t}^V + \gamma \lambda \sum_{l=0}^{\infty}(\gamma \lambda)^l \delta_{t+1 + l}^V \\
          & = \delta_{t}^V + A_{t+1}^{GAE}
\end{align*}
`$
...
之所以要区分$`A^{\theta'}(s_t,a_t)`$大于零和小于零这两种情况，是因为我们的最终目标是最大化$`J_{\mathrm{PPO2}}^{\theta'}(\theta)`$。

所以当$`A^{\theta'}(s_t,a_t)`$大于零时，说明这个动作好，则$`p_{\theta}\left(a_{t} | s_{t}\right)`$的值在最终目标导向下的本次参数更新完成后只会增加，增加的少无所谓（因为增加是一个正反馈的过程，只要本次有一点增加，下一次就会增加的更多），所以要限制每一次增加的步长（即重要性采样权重）不要太大，防止模型在好的方向上跑得太远。

当$`A^{\theta'}(s_t,a_t)`$小于零时，说明这个动作不好，则$`p_{\theta}\left(a_{t} | s_{t}\right)`$的值在最终目标导向下的本次参数更新完成后只会减小，减少的多无所谓（因为减少是一个负反馈的过程，本次减少的越多，下一次减少的就会越少），所以要限制每一次减少的步长（即重要性采样权重）不要太小，防止模型在不好的方向上原地不动（我们希望的是模型在不好的方向上也要向相反的方向上更新）。
